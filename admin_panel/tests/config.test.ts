...
const base = { VITE_CTMS_API_BASE_URL: '', VITE_CTMS_ENV: '', VITE_GOOGLE_MAPS_BROWSER_KEY: '' }

describe('build configuration', () => {
  it('defaults to development against the local API', () => {
    const config = readConfig({ ...base } as ImportMetaEnv)
  ...
  it('treats demo as staging, under the name people ask for it by', () => {
    const config = readConfig({ ...base, VITE_CTMS_ENV: 'demo' } as ImportMetaEnv)

    expect(config.environment).toBe('staging')
  })

  it('flags a production build still pointing at a developer laptop', () => {
    const config = readConfig({ ...base, VITE_CTMS_ENV: 'production' } as ImportMetaEnv)
